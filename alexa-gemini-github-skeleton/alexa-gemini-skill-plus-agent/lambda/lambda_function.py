...
# === SQS integration for local commands ===
import boto3

SQS_QUEUE_URL = os.environ.get("SQS_QUEUE_URL", "")

def _dispatch_to_sqs(utterance: str, meta: dict = None) -> bool:
    ...
    try:
        ...
        logger.exception("Erro ao publicar no SQS")
        return False
...
class AskGeminiIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, hi):
        ...
        lowered = utter.lower()
        if lowered.startswith(("abrir","tocar","executar","site","navegar","pressionar","atalho")):
            if _dispatch_to_sqs(utter, {"source":"alexa"}):
                return hi.response_builder.speak(_wrap_ssml("Comando enviado ao seu computador.")).response
